...
# C-PLAN: SUPREME FLEET COMMANDER - v4.5.1 (Marya Edition)
# ------------------------------------------------------------------------------
...
# 2. 🛰️ 環境判別 & 設定読み込み (CMD_NAMEを.envから取得)
# スクリプトのディレクトリ基準で .env を探す
ENV_PATH="./.env"
if [ -f "$ENV_PATH" ]; then
    # .env から CMD_NAME=XXX を抽出し、変数として取り込む
    export $(grep '^CMD_NAME=' "$ENV_PATH" | xargs)
fi

# 表示用司令部名の決定 ( .env になければホスト名を使用 )
DISPLAY_NAME=${CMD_NAME:-$(hostname)}

if [[ "$DISPLAY_NAME" == "Marya" ]]; then
...
# 7. 💥 出撃シークエンス開始
{
    ...
    for PRJ in "${PROJECTS[@]}"; do
        ...
        docker exec -T $TARGET_CONTAINER python manage.py $COMMAND_NAME --project "$PRJ" --platform "$TARGET_PF"
