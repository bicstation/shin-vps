...
async function fetchPostList(): Promise<WpPost[]> {
    // 🚨 修正点1: カスタム投稿タイプ 'avflash_post' を指定
    const WP_API_URL = `http://nginx-wp-v2/wp-json/wp/v2/avflash_post?_embed&per_page=5`; // 最新5件を取得

    try {
        const res = await fetch(WP_API_URL, {
            // 🚨 修正点2: Hostヘッダーを「Avflash」のドメインに設定
            headers: {
        ...
        <div style={{ fontFamily: 'Arial, sans-serif', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#f4f4f4' }}>
            ...
            <div style={{ display: 'flex', flexGrow: 1 }}>
                ...
                <main style={{ flexGrow: 1, padding: '20px' }}>
                    ...
                    {posts.length === 0 ? (
                        <p style={{ color: '#666' }}>現在、**Avflash**の記事は登録されていません。</p>
                    ) : (
                        <ul style={{ listStyleType: 'none', padding: 0 }}>
                            {posts.map((post) => (
