...
function resolveOptionLabel(
    items: CatalogOptionsData[keyof CatalogOptionsData] | undefined,
...
export default function CatalogHero({
    ...
    const makerLabel = resolveOptionLabel(
        options?.maker,
        filters.maker,
    )

    const brandLabel = resolveOptionLabel(
        options?.brand,
        filters.brand,
    )

    const seriesLabel = resolveOptionLabel(
        options?.series,
        filters.series,
    )

    const cpuLabel = resolveOptionLabel(
        options?.cpu,
        filters.cpu,
    )

    const gpuLabel = resolveOptionLabel(
        options?.gpu,
        filters.gpu,
    )

    const memoryLabel = resolveOptionLabel(
        options?.memory,
        filters.memory,
    )

    const storageLabel = resolveOptionLabel(
        options?.storage,
        filters.storage,
    )
    ...
    if (seriesLabel) {
        title = `${seriesLabel}のPC一覧`
    }
    else if (brandLabel) {
        title = `${brandLabel}のPC一覧`
    }
    else if (makerLabel) {
        title = `${makerLabel}のPC一覧`
    }
    else {
        title =
    ...
    if (
        ...
        if (cpuLabel) {
            specifications.push(`CPU: ${cpuLabel}`)
        }

        if (gpuLabel) {
            specifications.push(`GPU: ${gpuLabel}`)
        }

        if (memoryLabel) {
            specifications.push(`メモリ: ${memoryLabel}`)
        }

        if (storageLabel) {
            specifications.push(`ストレージ: ${storageLabel}`)
        }
    ...
    return (
        <section className={styles.catalogHero}>
            ...
            <div className={styles.catalogHeroVisual}>
                ...
                <Image
                    src="/images/catalog/catalog_hero.webp"
                    alt="PC商品一覧"
                    fill
                    priority
                    fetchPriority="high"
