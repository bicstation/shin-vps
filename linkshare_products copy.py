...
class LinkshareProduct(models.Model):
    # Django標準/必須フィールド (DBスキーマに合わせて null=True, blank=True を追加)
    id = models.BigAutoField(primary_key=True, verbose_name="ID") # bigint
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="作成日時")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新日時")
    
    # 【必須カラムとして追加・修正されたフィールド】
    sku_unique = models.CharField(max_length=255, unique=True, db_index=True, null=True, blank=True, verbose_name="ユニークSKU")
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="通常価格") 
    
    # 【🚨 追加フィールド 🚨】
    merchant_name = models.CharField(max_length=255, null=True, blank=True, verbose_name="マーチャント名")
    # 今回追加された api_source フィールド。NULL を許可します。
