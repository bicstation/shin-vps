...
export default async function SeriesVolumePage({ 
  ...
  // データソースの安全性確保
  const safeSource = Array.isArray(LIFESTYLE_GUIDE_DATA) ? LIFESTYLE_GUIDE_DATA : [];
  ...
  // --- 🌌 物理パスの解決 ---
  const filePath = path.join(process.cwd(), 'app/series/04-Life-Integration', `vol${volNum}.md`);
  ...
  const eyeCatchUrl = `/images/series/04-lifestyle/eye-catch-${eyeCatchPhase}.jpg`;
        ...
        <footer className="mt-40 pt-16 border-t border-zinc-900">
          <div className="grid grid-cols-1 sm:grid-cols-3 items-center gap-8">
            <div className="min-h-[60px]">
              {prevData && (
                <Link href={`/series/04-Life-Integration/${volNum - 1}`} className="series-nav-link text-left group">
            ...
            <div className="flex justify-center">
              <Link href="/series/04-Life-Integration" className="p-4 bg-zinc-900 border border-zinc-800 rounded-full hover:border-emerald-500/50 transition-all group">
            ...
            <div className="min-h-[60px]">
              {nextData && (
                <Link href={`/series/04-Life-Integration/${volNum + 1}`} className="series-nav-link text-right group">
