...
"""
FILE:
acquisition/sources/scraping/geekom/observe.py
...
def observe_price(
    ...
    """
    Observe explicit GEEKOM price Reality.

    GEEKOM collection Reality:
    ...
    The same source representation is
    preserved together with explicit
    numeric Reality.
...
def observe_images(
    ...
    """
    Observe GEEKOM Product Images.

    Primary image:

        .product-gallery__media.is-selected img

    Additional gallery images are preserved
...
def observe_feature_chart(
    ...
    """
    Preserve feature-chart Reality.

    This supports sites using:

        <feature-chart>

    GEEKOM may not use this structure,
    but the generic observer preserves it
    when present.
...
def observe_html_tables(
    ...
    GEEKOM product pages use an explicit
    HTML table for specifications:

        <table class="pf-inner-table">

            <tr>
                <th>サイズ</th>
                <th>135 x 132 x 46.9 mm</th>
            </tr>
    ...
    The complete table is preserved as
    observable text.
...
def observe_tables(
    ...
    """
    Observe explicit specification
    Reality containers.
...
def observe_product_name(
    ...
    """
    Observe explicit GEEKOM Product Name.
...
def run() -> None:
    """
    Execute GEEKOM Observation Runtime.
