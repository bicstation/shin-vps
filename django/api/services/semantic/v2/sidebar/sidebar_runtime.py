# -*- coding: utf-8 -*-
# /home/maya/shin-dev/shin-vps/django/api/services/semantic/v2/sidebar/sidebar_runtime.py

from api.services.semantic.v2.topology.topology_runtime import (
    build_topology_runtime,
)

from api.services.semantic.v2.authority.authority_runtime import (
    build_authority_runtime,
)

from api.services.semantic.v2.traversal.traversal_builder import (
    build_traversal_runtime,
)

from api.services.semantic.v2.navigation.navigation_rules import (
    is_secondary_group,
)


# ==========================================================
# COUNT
# ==========================================================

def calculate_product_count(

    products,

    group_slug,
):

    return len([

        product

        for product in products

        if group_slug in

        product.get(
            "matched_groups",
            []
        )
    ])


# ==========================================================
# SIDEBAR
# ==========================================================

def build_sidebar_runtime():

    authority = (
        build_authority_runtime()
    )

    topology = (
        build_topology_runtime()
    )

    traversal = (
        build_traversal_runtime()
    )

    products = (

        traversal.get(
            "products",
            []
        )
    )

    filters = []

    # ======================================================
    # GROUPS
    # ======================================================

    for group in topology.get(
        "groups",
        []
    ):

        group_slug = (
            group.get(
                "slug"
            )
        )

        parent_group = (
            group.get(
                "parent_group"
            )
        )

        # --------------------------------------------------
        # Sidebar Policy
        # --------------------------------------------------

        if not is_secondary_group(
            parent_group
        ):
            continue

        # --------------------------------------------------
        # Reality Count
        # --------------------------------------------------

        product_count = (

            calculate_product_count(

                products,

                group_slug,
            )
        )

        # --------------------------------------------------
        # Runtime
        # --------------------------------------------------

        filters.append({

            "slug":
                group_slug,

            "name":
                group.get(
                    "name"
                ),

            "type":
                group.get(
                    "type"
                ),

            "icon":
                group.get(
                    "icon"
                ),

            "color":
                group.get(
                    "color"
                ),

            "parent_group":
                parent_group,

            "attribute_count":

                len(
                    group.get(
                        "attributes",
                        []
                    )
                ),

            "product_count":
                product_count,
        })

    # ======================================================
    # SORT
    # ======================================================

    filters = sorted(

        filters,

        key=lambda x: (

            x.get(
                "parent_group",
                ""
            ),

            -x.get(
                "product_count",
                0
            ),

            x.get(
                "name",
                ""
            ),
        )
    )

    # ======================================================
    # DEBUG
    # ======================================================

    if filters:

        print(
            "🔥 SIDEBAR SAMPLE",
            filters[0]
        )

    # ======================================================
    # PAYLOAD
    # ======================================================

    return {

        "filters":
            filters,

        "semantic_schema_version":

            authority.get(
                "semantic_schema_version"
            ),

        "authority_version":

            authority.get(
                "authority_version"
            ),

        "semantic_authority":

            authority.get(
                "semantic_authority"
            ),

        "ready":
            True,
    }