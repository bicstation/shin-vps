...
# 💡 役割: 商品取得 -> 価格履歴保存 -> AIスペック解析 -> サイトマップ更新
# ==============================================================================
...
# 対話プロンプトに 'y' を自動入力して実行
echo "y" | run_django record_price_history --all

# ------------------------------------------------------------------------------
# 🤖 STEP 3: AIスペック解析 (未解析分を自動的に埋める)
# ------------------------------------------------------------------------------
echo -e "\n\e[36m🤖 [3/4] AIスペック解析を実行中 (未解析最大500件)...\e[0m"
# 💡 4時間おきに実行するため、1回あたり500件に制限するのが現実的かつ安全です。
# 500件なら約2〜3時間で終わり、次回の実行と重なるリスクを抑えられます。
