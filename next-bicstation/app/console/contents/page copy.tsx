...
const APP_ID = 'bicstation-main';

// --- 2. Source Data (大カテゴリ構造に拡張) ---
const SOURCE_DATA = {
  // ここが「大カテゴリ（Parent Category）」
  parentTitle: "物理要塞：1円あたりの演算密度を最大化し、家全体をPCにする",
  categories: {
    // ここから＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊
      bto: {
    id: "bto",
...
export default function ContentMappingPage() {
  ...
  const [stats, setStats] = useState({ categories: 0, series: 0, episodes: 0 });
  ...
  useEffect(() => {
    if (!user) return;
    const q = collection(db, 'artifacts', APP_ID, 'public', 'data', 'episodes');
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setStats({
        ...
        series: new Set(items.map(i => i.seriesLabel)).size
      });
  ...
  const syncToDatabase = async () => {
    ...
    try {
      ...
      // 大カテゴリ階層をループ
      Object.entries(SOURCE_DATA.categories).forEach(([catId, catData]) => {
        catData.phases.forEach((phase) => {
          phase.episodes.forEach((episode) => {
            const docId = `${catId}-${episode.ep}`;
            const docRef = doc(db, 'artifacts', APP_ID, 'public', 'data', 'episodes', docId);
            
            batch.set(docRef, {
              parentTitle: SOURCE_DATA.parentTitle, // 大カテゴリを保存
              categoryId: catId,
              categoryTitle: catData.title,
              seriesLabel: phase.label,
  ...
  const filteredEpisodes = dbContent.filter(ep => 
    ep.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ep.categoryTitle?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-6">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-xl">
          <div className="space-y-1">
            <div className="flex items-center space-x-2 text-blue-400 text-sm font-bold uppercase tracking-tighter">
              <Tag className="w-4 h-4" />
              <span>Project: {SOURCE_DATA.parentTitle.split('：')[0]}</span>
            </div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-400 to-emerald-400 bg-clip-text text-transparent">
              Content Mapping Engine
            </h1>
            <p className="text-slate-400 text-sm">
              {SOURCE_DATA.parentTitle}
            </p>
          </div>
          ...
          <button
            ...
            className={`flex items-center space-x-3 px-8 py-4 rounded-2xl font-bold transition-all ${
              status === 'processing' ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500 text-white active:scale-95'
            }`}
          >
            {status === 'processing' ? <Loader2 className="w-5 h-5 animate-spin" /> : <UploadCloud className="w-5 h-5" />}
            <span>同期実行</span>
          </button>
        </div>
        ...
        <div className="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden">
          <div className="p-6 border-b border-slate-800 flex items-center justify-between">
            <h2 className="font-bold flex items-center space-x-2 text-slate-300">
            ...
            <input 
              ...
              className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-sm w-64 focus:border-blue-500 outline-none"
            />
          </div>
          
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-950/50 text-xs text-slate-500 uppercase font-bold">
                <tr>
                  <th className="p-4">Parent Project</th>
                  <th className="p-4">Category / Series</th>
              ...
              <tbody className="divide-y divide-slate-800/50">
                {filteredEpisodes.map((ep) => (
                  <tr key={ep.id} className="hover:bg-slate-800/20 text-sm">
                    <td className="p-4 text-xs text-slate-500 italic">{ep.parentTitle?.split('：')[0]}</td>
                    <td className="p-4">
                      <div className="text-blue-400 font-bold text-[10px]">{ep.categoryTitle}</div>
                      <div className="text-slate-500 text-[10px]">{ep.seriesLabel}</div>
                    </td>
                    <td className="p-4 font-medium text-slate-200">{ep.title}</td>
                    <td className="p-4">
                      <span className="mx-auto flex items-center justify-center text-[9px] text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-full w-fit">
