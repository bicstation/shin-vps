...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 3. Geminiによる執筆プロンプト作成（メタ情報を排除）
        prompt = f"""
        あなたはテック系ブログ『Bicstation』の専門レビュアーです。
        以下の製品データに基づき、読者が読みやすく、購買意欲をそそるWordPress用のHTML記事を書いてください。

        【重要命令】
        - 「思考プロセス」「タイトル作成」「執筆者」「カテゴリ」などのメタ情報は絶対に含めないでください。
        ...
        4. 箇条書きが必要な場合は <ul><li> タグを使用。
        5. 文末に必ず「この製品の詳細は、以下のリンクからご確認いただけます」という趣旨の結びの言葉を入れてください。
        """
        ...
        try:
            ...
            # メタ情報や数字から始まる解説行（1. タイトルなど）を徹底排除
            filtered_lines = []
            exclude_keywords = ["思考プロセス", "執筆者", "カテゴリ", "ペルソナ", "ターゲット", "構成", "ステップ"]
            ...
            # 5. 画像＆Wボタン付き豪華リッチカード（レスポンシブ設計）
            # PCでは横並び、スマホでは縦並びになる独自HTMLです
            custom_card_html = f"""
            <div style="margin: 40px 0; padding: 25px; border: 1px solid #e5e7eb; border-radius: 16px; background-color: #ffffff; box-shadow: 0 4px 20px rgba(0,0,0,0.08); font-family: 'Helvetica Neue', Arial, sans-serif;">
                    ...
                    <div style="flex: 2; min-width: 250px;">
                        ...
                        <div style="display: flex; gap: 12px; margin-top: 20px; flex-wrap: wrap;">
                            <a href="{product.url}" target="_blank" rel="noopener noreferrer" 
                               style="flex: 1; min-width: 140px; background-color: #0062ff; color: #ffffff; text-align: center; padding: 14px 10px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 0.95em; box-shadow: 0 4px 6px rgba(0,98,255,0.2); transition: 0.2s;">
                               公式サイト ＞
                            </a>
                            <a href="{bic_detail_url}" 
                               style="flex: 1; min-width: 140px; background-color: #1f2937; color: #ffffff; text-align: center; padding: 14px 10px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 0.95em; box-shadow: 0 4px 6px rgba(0,0,0,0.1); transition: 0.2s;">
                               Bicstation詳細 ＞
                            </a>
                        </div>
                    </div>
            ...
            # 記事全体の組み立て（アイキャッチ画像 + 本文 + 豪華カード）
            img_html = f'<img src="{media_url}" alt="{product.name}" class="wp-image-{media_id} size-large" style="margin-bottom: 30px; border-radius: 12px;" />' if media_url else ""
            ...
            if wp_res.status_code == 201:
                ...
                self.stdout.write(f"Bicstation詳細URL: {bic_detail_url}")
            else:
