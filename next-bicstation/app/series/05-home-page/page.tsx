...
/**
 * 🌐 自律型プラットフォーム・Web構築ガイド (Series Index)
 * 🛡️ Maya's Logic: Digital Real-Estate Layer V12.1
 * 物理パス: app/series/05-web-creation/page.tsx
 * 修正内容: 第5章（Web構築・情報発信）の追加 & 安全装置実装
 */

import React from 'react';
import Link from 'next/link';
// データ実体 (WEB_CREATION_DATA が export されている前提)
import { WEB_CREATION_DATA } from './data';

export const dynamic = 'force-static';

export default function WebCreationSeriesPage() {
  const phases = ["初級編", "中級編", "上級編"];

  // ✅ 安全装置: データが存在しない場合に備える
  const safeData = WEB_CREATION_DATA || [];

  // アイキャッチURLを決定するヘルパー関数
  const getEyeCatchUrl = (vol: number) => {
    const phase = Math.ceil(vol / 10);
    return `/images/series/05-web-creation/eye-catch-${phase}.jpg`;
  };

  return (
    <div className="max-w-6xl mx-auto py-16 px-6 bg-gray-900 min-h-screen text-gray-100">
      {/* ヒーローセクション - ゴールド・アンバー系の輝き */}
      <header className="mb-20 text-center relative py-10">
        <div className="absolute inset-0 bg-yellow-500/5 blur-3xl rounded-full" />
        <h1 className="relative text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-amber-400 to-orange-400 mb-6 tracking-tighter">
          自律型プラットフォーム・Web構築ガイド
        </h1>
        <p className="relative text-gray-400 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
          消費側から供給側へ。最新のNext.js 15やDjangoを用いたフルスタック開発、SEO、そして個人の知見を資産に変えるデジタル発信拠点の作り方を伝授。
        </p>
      </header>
          ...
          <section key={phase} className="relative">
            ...
            <div className="sticky top-0 bg-gray-900/90 backdrop-blur-md py-6 z-20 mb-10 border-b border-gray-800 flex items-end justify-between">
              <h2 className="text-3xl font-bold flex items-center tracking-tight">
                <span className="w-1.5 h-10 bg-gradient-to-b from-yellow-500 to-amber-600 mr-4 rounded-full shadow-[0_0_15px_rgba(245,158,11,0.5)]"></span>
                {phase}
              </h2>
              <span className="text-gray-500 font-mono text-sm mb-1 uppercase tracking-widest">
                Web_Build_Phase_{phase === "初級編" ? "01" : phase === "中級編" ? "02" : "03"}
              </span>
            </div>
              ...
              {safeData
                ...
                .map((item) => {
                  ...
                  return (
                    <Link 
                      key={item.vol} 
                      href={`/series/05-web-creation/${item.vol}`} 
                      className="group relative overflow-hidden bg-gray-800/30 border border-gray-700/50 rounded-2xl hover:border-yellow-500/50 transition-all duration-500 shadow-xl hover:shadow-yellow-500/10 flex flex-col h-full"
                    >
                      {/* カード内アイキャッチ画像 */}
                      <div className="relative h-32 w-full overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4">
                          <span className="text-[10px] font-mono font-bold bg-yellow-600/80 text-white px-2 py-0.5 rounded backdrop-blur-sm">
                      ...
                      <div className="p-6 flex-grow flex flex-col justify-between">
                        <div>
                          <h3 className="text-xl font-bold text-gray-100 group-hover:text-yellow-400 transition-colors mb-3 leading-snug">
                            {item.title}
                          </h3>
            ...
            {safeData.filter(d => d.category === phase).length === 0 && (
              <div className="text-center py-10 border border-dashed border-gray-800 rounded-2xl">
                <p className="text-gray-600">Site Initialization... Phase 05 is on standby.</p>
              </div>
            )}
          </section>
        ))}
      ...
      <footer className="mt-32 border-t border-gray-800 pt-10 text-center text-gray-600 text-sm">
        <p>© 2026 SHIN-VPS & BICSTATION. Digital Estate Design by Maya.</p>
      </footer>
    </div>
  );
}