...
# traceback はデバッグ専用のため、通常は削除またはコメントアウト推奨
import traceback # デバッグのためにインポート

# 共通モジュールからのインポート
# ★修正点 1: entity_manager は adult/ の下にあるため、パスを修正★
from api.utils.adult.entity_manager import get_or_create_entity 
# ★修正点 2: fanza_normalizer は adult/ の下にあるため、パスを修正★
...
# ロガーのセットアップ
# ★修正点 3: ファイルが adult/ の下にあるため、ロガー名を adult.import_fanza に変更★
logger = logging.getLogger('adult.import_fanza')
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # ロガー名が 'api_utils' であれば、全ての子ロガーに影響する
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    for entity_type, names in all_entities.items():
                        ...
                        # api.utils.adult.entity_manager.get_or_create_entity を使用してエンティティを作成/更新
                    ...
                    product_id_uniques = [f'{self.API_SOURCE}_{api_id}' for api_id in api_ids]

                    # AdultProduct を参照
                    db_products = AdultProduct.objects.filter(
                        product_id_unique__in=product_id_uniques
    ...
    def _synchronize_relations(self, relations_list: list[dict], product_pk_map: dict):
        ...
        # AdultProduct の中間テーブルでの FK 名を取得（通常はモデル名の小文字+ _id）
        adult_product_fk_name = AdultProduct._meta.model_name + '_id' # 例: 'adultproduct_id' 
        ...
        for rel in relations_list:
            ...
            for genre_id in rel['genre_ids']: # ここはID（PK）である必要がある
                genre_relations.append(
                    # AdultProduct の中間テーブルを参照し、フィールド名を adultproduct_id に変更
    ...
    def update_product_counts(self, stdout):
        ...
        # --------------------------------------------------------
        # AdultProduct の中間テーブルでの FK 名を取得（Raw SQL 削除で使用したものと同じ）
        adult_product_fk_name = AdultProduct._meta.model_name + '_id' 
        
        # 1. 女優 (Actress) のカウント更新
        # AdultProduct の中間テーブルを参照
        actress_count_sq = Subquery(
            AdultProduct.actresses.through.objects
            .filter(actress_id=OuterRef('pk'))
            .values('actress_id')
            .annotate(count=Count(adult_product_fk_name)) # カウント対象は adultproduct_id
            .values('count'),
