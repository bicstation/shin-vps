...
while true; do
    ...
    case $CHOICE in
        # --- 1. ADULT SECTION (Original Case 2, 20, 21, and new 22) ---
        10)
            echo -e "\n${YELLOW}--- 🔞 FANZA / DUGA インポート管理 ---${RESET}"
            echo "1) 一括 / 2) DUGA / 3) FANZA / 4) F-Reset / 5) D-Reset / 6) Normalize / 7) RawDelete"
            read -p ">> " ADULT_CHOICE
            case $ADULT_CHOICE in
                ...
                7) read -p "生データを削除しますか？ (y/N): " CLEAN_RAW_CONFIRM
...
EOF
        ...
        # --- 2. PC & SHOP SECTION (Original Case 3, 4, 18, 19) ---
        20) # Original Case 3: このロジックは一切削っていません
...
EOF
        ...
        # --- 3. AI & NEWS SECTION (Original Case 6, 7, 16, 17) ---
        30) # Original Case 6
